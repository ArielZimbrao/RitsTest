import React from 'react'
import './Component8.scss'

export class Component8 extends React.Component {
    render() {
        return (
            <div className="component8-container">

                <div className="clients">
                    <div className="viverra">
                        <div className="title">
                            <span>Viverra</span>
                        </div>
                        <div className="line"></div>
                        <div className="description">
                            <span>Sed varius cursus fringilla. Interdum: </span>
                            et malesuada fames ac ante ipsum primis in faucibus. Suspendisse feugiat orci sed velit pulvinar pretium.
                        </div>
                        <div className="hsm"></div>
                    </div>
                    <div className="pretium">
                        <div className="title">
                            <span>Pretium</span>
                        </div>
                        <div className="line"></div>
                        <div className="description">
                            <span>Nullam tincidunt at diam at faucibus. Nunc euismod felis in lectus cursus, eget laoreet est sagittis: </span>
                            Etiam ut pharetra urna, interdum malesuada quam. Aenean vel imperdiet urna.
                        </div>
                        <div className="harvard"></div>
                    </div>
                    <div className="mauris">
                        <div className="title">
                            <span>Mauris</span>
                        </div>
                        <div className="line"></div>
                        <div className="description">
                            <span>Hac habitasse platea dictumstl tiam tortor diam: </span>
                            ornare vel libero eget, posuere malesuada lectus. Vestibulum quis lacus lacinia, porttitor felis aliquet, sollicitudin neque.
                        </div>
                        <div className="hsm-university"></div>
                    </div>
                </div>
            </div>
        )
    }
}